...
<script>
	function id_check(){
		window.open("id_check.jsp","id_check","x=150,y=200,width=350,height=230");
	}
	function post_check(){
	...
	function send(){
		...
		//회원Id userid(text)
		var pattern=/^[A-Za-z]{1}[A-Za-z0-9]{4,16}$/; //첫번째 자리가 영문이고 영문, 숫자 입력 가능한 5~16
		if(!userinfo.userid.value){
			alert("아이디를 입력하세요");
			userinfo.userid.focus();
			return;
		}
		else if(!pattern.test(userinfo.userid.value)){
			alert("5~16자 이내의 영문이나 숫자를 입력하세요");
			userinfo.userid.focus();
			return;
		}
		...
		else if(!pattern.test(userinfo.passwd.value)){
			alert("6~12자 이내의 영문이나 숫자를 입력하세요");
			userinfo.passwd.focus();
			return;
		}
		//비밀번호확인 repasswd(password)
		if(!userinfo.repasswd.value){
			alert("비밀번호를 확인하세요");
			userinfo.repasswd.focus();
			return;
		}
		else if(userinfo.passwd.value!=userinfo.repasswd.value){
			alert("비밀번호가 맞지 않습니다");
			userinfo.repasswd.focus();
			return;
		}
...
<tr>
  ...
  <td width="80%" valign="top">&nbsp;<img src="./img/title1.gif" ><br>    
	<form name="userinfo" method="post" action="userinfo_insert_pro.jsp">
					...
					<td bgcolor="#7AAAD5">            
						...
						<table cellpadding=3 cellspacing=1 border=0 width=100%>
							<tr>
								...
								<TD BGCOLOR=WHITE>
									<input type=text name=name size=16 maxlength=20>성명은 빈칸없이 입력하세요.
								</td>
							</tr>
							<tr>
								...
								<TD BGCOLOR=WHITE>
									<table cellspacing=0 cellpadding=0>
										<tr>
											<td align=absmiddle>
												<input type=text name=userid size=12 maxlength=16 style="width:120" readonly>
											</td>
							...
							<tr>
								...
								<TD BGCOLOR=WHITE><input type=password name=repasswd size=8 maxlength=12 style="width:80">비밀번호 확인을 위해서 비밀번호를 한번 더 입력해주세요. </td>
							</tr>
							<tr>
							...
							<tr>
								...
								<TD BGCOLOR=WHITE>
									<input type=text name=tel size=13 maxlength=13>
								</td>
							</tr>
							<tr>
